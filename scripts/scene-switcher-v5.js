...
v4 to 5 changes:
	SS.obs.call('SetCurrentScene', {'scene-name': scene})
to	SS.obs.call('SetCurrentProgramScene', { sceneName : scene })

	let opts = {source, mute};
to	let opts = {inputName: source, inputMuted: mute};
for	SS.obs.call('SetInputMute', opts)
...
function obs_scene_change(index) {
	...
	//SS.obs.call('SetCurrentScene', {'scene-name': scene})
	SS.obs.call('SetCurrentProgramScene', { sceneName : scene })
...
function obs_mute_source(source, mute = true) {
	//let opts = {source, mute};
	let opts = {inputName: source, inputMuted: mute};

	SS.obs.call('SetInputMute', opts)
