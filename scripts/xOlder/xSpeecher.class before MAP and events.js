...
{	// scope
	...
	class Speecher {
		...
		queueID = 0;	// incremented every time speak() is called
		...
		clear() {
			...
			// actually NO - you can pass in an utterance, object or string
			// I want to return an ID
		...
		_is_voice(v) {
			...
			// returns filtered voice, pitch, volume, rate and utterance events
		...
		_sayQueueProcess(immediate = false) {
			...
			if (pack) {
				...
				//this.utterance.lang = "en-GB";  // TEST
				console.log("utterance", this.utterance);

					// emit that we're about to speak
				this.cancelNextSpeak = false;
				this.emit('beforespeak');
				if (this.cancelNextSpeak) {	// then can CANCEL the speak
